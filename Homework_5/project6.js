...
bool IntersectShadowRay(Ray ray)
{
	...
	for ( int i=0; i<NUM_SPHERES; ++i ) 
	{
		...
		float discriminant = pow(dot(ray.dir, (ray.pos - sphere.center)), 2.0) - (dot(ray.dir, ray.dir) * (dot((ray.pos - sphere.center), (ray.pos - sphere.center)) - pow(sphere.radius, 2.0))); 
...
vec4 RayTracer( Ray ray )
{
	...
	if ( IntersectRay( hit, ray ) ) 
	{
		...
		for ( int bounce=0; bounce<MAX_BOUNCES; ++bounce ) 
		{
			...
			{
				// The refleciton ray did not intersect with anything, so use the environment color
	...
	{
		return vec4( textureCube( envMap, ray.dir.xzy ).rgb, 0 );	// env color
	}
}
`;